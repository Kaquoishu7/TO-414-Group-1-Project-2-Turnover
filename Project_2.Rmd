...
# Use for L, SVM, and DT
trainNorm1 = turnover[-testIndices1,]
testNorm1 = turnover[testIndices1,]

trainNormUS1 = trainNorm1
trainNormUS1$turnover = as.factor(trainNormUS1$turnover)

trainNormUS1 = upSample(trainNormUS1, trainNormUS1$turnover)
trainNormUS1$turnover = ifelse(trainNormUS1$turnover == "0", 0 , 1)
trainNormUS1$Class = NULL
...
testNorm1KNN =turnover_norm[testIndices1,
                          -match("turnover",names(turnover_norm))]
# trainNormKNNUS1 = trainNorm1KNN
# trainNormKNNUS1$turnover = as.factor(trainNormKNNUS1$turnover)
# 
# trainNormKNNUS1 = upSample(trainNormKNNUS1,
#                            trainNormKNNUS1$turnover)
# trainNormKNNUS1$turnover = ifelse(trainNormKNNUS1$turnover == "0",
#                                   0 , 1)
...
```{r}
# set.seed(300)
# #kList = 1:round(sqrt(nrow(trainNorm1KNN)))
...
set.seed(300)
kList = 1:round(sqrt(nrow(trainNorm1KNN)))
grid <- expand.grid(.k = kList[kList %% 2 == 1])
...
KNNmodel <- train(as.factor(turnover) ~ .,
                        ...
                        tuneGrid = grid)
KNNmodel
KNNpred <- predict(KNNmodel, testReg1)
#KNNpred
```
...
svm_model <- train(as.factor(turnover) ~., data = trainReg1, method = "svmLinear",
                 trControl=ctrl,
...
testpred <- predict(svm_model, testReg1)
testpred

table(testpred, testReg1$turnover)
...
```{r, cache = TRUE}
#prop.table(table(trainNormUS1$turnover))
#prop.table(table(testNorm1$turnover))
dt_model <- C5.0(as.factor(turnover) ~ ., data = trainRegUS1)
dt_predict <- predict(dt_model, testReg1)
plot(dt_model, subtree = 1)
library(gmodels)
CrossTable(testReg1$turnover, dt_predict,
           prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE,dnn = c('Actual Turnover', 'Predicted Turnover'))
