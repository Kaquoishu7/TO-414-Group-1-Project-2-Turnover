...
turnoverUS_labels <- turnoverUS$turnover
```
...
grid <- expand.grid(sigma = c(.01, 0.10, .015, 0.2),
...
```{r}
SVMPred <- predict(svm_model, turnoverUS)

table(SVMPred, turnoverUS$turnover)

confusionMatrix(as.factor(SVMPred),
                as.factor(turnoverUS_labels))

CrossTable(SVMPred, turnoverUS$turnover,
           prop.chisq = FALSE, prop.c = FALSE,
           prop.r = FALSE,
           dnn = c('Predicted Turnover', 'Actual Turnover'))
```
...
```{r}
turnover_pred <- data.frame(logPredictionsBinary1, KNNPred, ANNPred, SVMPred, dt_predict, testRegLabels1)
...
turnover_stack_dt <- C5.0(as.factor(testRegLabels1) ~ .,
...
CrossTable(turnover_stack_model,
           turnover_stack_test$testRegLabels1,
           prop.chisq = FALSE, prop.c = FALSE,
           prop.r = FALSE,
           dnn = c('Predicted Turnover', 'Actual Turnover'))
confusionMatrix(as.factor(turnover_stack_model),
                as.factor(turnover_stack_test$testRegLabels1))
```
